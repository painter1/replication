#!/bin/env python
"""Encapsulate access to the p2p search."""
import json
import re
import urllib2
import sys
    ...
    def __constraints_to_str(self, constraints, **defaults):
        ...
        if constraint_dict:
            for key, value in constraint_dict.items():
                ...
                if key=='project':  #jfp: convert cmip5 to CMIP5, as web server wants CMIP5
                    if isinstance(value,list): value=[v.upper() for v in value]
...
just the first "N" records."""
    ...
    def datasets(self, batch_size=__MAX_RESULTS, **constraints):
        ...
        try:
            ...
            # jfp added this exception handling; try to go on...
...
return {facet:{facet_value:count}}"""
    ...
    def show(dictio, return_str=False):
        ...
        if type(dictio) is dict:
            strng = json.dumps(dictio, sort_keys=True, indent=2)
        else:
            strng = str(dictio)   #jfp.  also changed varname from str to strng
        if return_str: return strng
        print strng
...
def main(argv=None):
    ...
    try:
        args, lastargs = getopt.getopt(argv, "f:h", ['help','facet=','query=', 'list-datasets'])
    except getopt.error:
